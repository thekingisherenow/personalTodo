...
function Completed({completedTodolist}) {
  const [completedTodolist, setTodolist] = useState([])
  ...
  return (
    <div className='p-2 m-3 '>
      ...
      <span className='mx-4 text-normal text-lg text-center my-auto '>{completedTodolist.length}</span>
      ...
      {open && <div className='border p-3 m-7 border-orange-700'>
            {completedTodolist.length? completedTodolist.map((todo,index)=>{
                return(
